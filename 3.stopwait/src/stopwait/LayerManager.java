...
public class LayerManager { //Layer瑜� 愿�由ы빐二쇰뒗 �겢�옒�뒪(怨꾩링媛� �뿰寃�)
	...
	private ArrayList<BaseLayer> mp_aLayers = new ArrayList<BaseLayer>() ;//chatapp,socket,IPCDIG�뱾�쓣 ���옣
	...
	public void ConnectLayers(String pcList){
		MakeList(pcList);
		LinkLayer(mp_sListHead); //mPList�뿉 �꽔�� 媛믩뱾怨� �뿰寃�
	}

	private void MakeList(String pcList){ //�뱾�뼱�삤�뒗 Layer �씠由꾩쓣 token�쑝濡� �옒由�
		StringTokenizer tokens = new StringTokenizer(pcList, " ");
		
		for(; tokens.hasMoreElements();){
			_NODE pNode = AllocNode(tokens.nextToken()); //媛� �넗�겙�쓣 �끂�뱶濡� �븷�떦
			AddNode(pNode);//�빐�떦 �넗�겙�쓽 �끂�뱶瑜� mp_list濡� �뿰寃고빐以�
			
		}	
	}

	private _NODE AllocNode(String pcName){ //媛� �넗�겙�쓽 �끂�뱶�뱾�쓣 �뿰寃고빐以�
	...
	private void LinkLayer(_NODE pNode){ //怨꾩링 媛� �뿰寃고빐以�
		...
		while(pNode != null){
			if( pLayer == null)
				...
				if(pNode.token.equals("("))
					...
					switch(cMode){
					case '*':
						Top().SetUpperUnderLayer( pLayer ); //�뼇諛⑺뼢�뿰寃�
						break;
					case '+':
						Top().SetUpperLayer( pLayer ); //�쐵諛⑺뼢 �뿰寃�
						break;
					case '-':
						Top().SetUnderLayer( pLayer ); // �븘�옒諛⑺뼢 �뿰寃�
						break;
					}					
				}
			}
